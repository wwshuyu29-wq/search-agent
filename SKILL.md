...
Codex installs this repository as a **single top-level Skill**. Sixteen curated **internal specialists** resolve through `specialists/catalog.json`; `specialists/vendor.lock.json` preserves upstream provenance. Update manually with `git pull origin main` and `bash install.sh`. Specialist output never bypasses Citation Audit or T1-T9 gates.
...
3. **多源分层搜索**：整合 Firecrawl（英文深度）、realtime-search（Brave 官方 + 百度中文）、baidu-search、enterprise-search（内网）、finance-rss-reader（65+ 个财经/产业/AI 媒体源），按置信度 5 层分层调用
4. **多格式内容处理**：支持 PDF（OCR）、HTML 正文提取、知识库文档（ku-doc-manage）、URL 全文抓取
5. **深度写作闭环**：CitationAudit 只批准有真实来源的 claim；生成三套 OutlinePlan 并等待用户选择/组合/覆盖章节；无在线 LLM SDK 时使用确定性 Evidence Synthesis Writer，随后依次通过 OutlineCompliance、Humanizer、IntegrityDiff 和终稿审核
...
**子 agent 与节点契约**：Step 0-3 不是靠关键词和模板硬跑。每次执行必须遵守 `references/agent-nodes.md`；每个子 agent 如何按"输入 -> LLM判断 -> skill/tool调用 -> 输出artifact -> 进入下一步条件"推进，见 `references/node-playbook.md`；Codex 中 LLM 如何调用、是否需要 API Key、如何并行/兜底，见 `references/codex-execution.md`；可执行契约维护在 `lib/workflow_contracts.py`。先做 LLM 语义理解，再做专家 skill 前置探针，再用规则/关键词分类器校验，最后由框架组合器生成可审核方案。`lib/intent_classifier.py` 和 `lib/framework_combinator.py` 只是 CLI 兜底与可复现 guardrail，不得替代 LLM 语义理解和专家 skill 前置。增长类、单一金融数字等分类依赖整句语义判断；关键词只是语义信号，不是封闭枚举。
...
Step 0 的目标不是猜关键词，而是识别用户要支持的业务决策。执行顺序：

1. **LLM 语义理解**：抽取调研对象、用户决策、读者、时间范围、输出形态、证据需求和不确定项；增长/金融数字/竞品/营销方案等意图按整句语义判断，不能只看关键词。
2. **专家 skill 前置**：根据触发条件先用垂直 skill 帮助定框架或短路回答。
3. **规则/关键词分类器**：使用 `lib/intent_classifier.py` 做可复现校验和 CLI fallback。
4. **框架组合器**：使用 `lib/framework_combinator.py` 选择最小可用组合。
5. **审核卡片**：说明语义判断依据、拟使用 skill、框架选择原因和可调整项，然后停下等待确认。

详细子 agent 设定、输入输出、禁止事项和报告生成逻辑见 `references/agent-nodes.md`。若 LLM 语义理解与关键词分类器冲突，以 LLM 判断为主，并在审核卡片说明冲突与取舍。
...
1. **语义建模**：用 LLM 先把用户话语改写为"调研对象 + 决策问题 + 证据需求"。
2. **专家前置**：命中营销/金融/创业/单一数字触发器时先调用对应 skill，或在审核卡片中列为 Step 1/2 必调用项。
3. **判断复杂度**：命中"全面 / 系统 / 多维度 / 深入 / 综合"或问题长度 > 50 字 → 复杂问题，考虑推荐**框架组合**。
4. **查询路由表**（见 `references/frameworks.md` 第一节），结合 `lib/intent_classifier.py` 得分校验主框架。
5. **应用引导性扩框规则**（意图单一时主动建议叠加一个补框架）。
6. **输出【审核卡片】并停下**，等用户回复"确认"或修改意见。
...
║   [x] 财经 RSS 聚合（Reuters / MarketWatch / 财新 / 21 世纪经济报道 / 证券时报等 65+ 个源）
...
- 环境变量：`FIRECRAWL_API_KEY`（不要提交到 GitHub；在本机 shell 配置或 CI secret 中设置）
...
**工具 6：财经 RSS 聚合**（65+ 个高质量财经/产业/AI 媒体源）
...
Layer 5（快讯参考 medium）：财经 RSS 聚合
   → 工具：finance-rss-reader（65+ 个源，生产调研默认全量扫描）
...
│   └── finance-rss-reader/     # 内嵌 RSS 拉取组件（65+ 源）
├── bin/
...
- 本地脚本 `lib/finance-rss-reader/scripts/rss_fetch.py`：65+ 个财经/产业/AI 媒体源聚合（内嵌）
